...
struct semaphore binSemArray[MAX_BSEM]; 
...
procinit(void)
{
  ...
  initlock(&bsem_lock, "binSemArray"); //task 4.1
  for(p = proc; p < &proc[NPROC]; p++) {
...
//binary semaphore alloc
int bsem_alloc(void) {
  acquire(&bsem_lock);
	for (int i = 0; i < MAX_BSEM; i++) {
    if (binSemArray[i].is_used == 0) {
      //initialize
      binSemArray[i].is_used = 1;
      binSemArray[i].id = allocsid();
      binSemArray[i].is_locked = 0;
      release(&bsem_lock);
      return binSemArray[i].id;
		} 
  }
  release(&bsem_lock);
  return -1;
...
void bsem_free(int bsem_id){
  acquire(&bsem_lock);
  for (int i = 0; i < MAX_BSEM; i++) {
    if (binSemArray[i].id == bsem_id) { //found the semaphore
      binSemArray[i].is_used = 0;
      binSemArray[i].id = 0;
      binSemArray[i].is_locked = 0; 
      release(&bsem_lock);
      return; 
...
void bsem_down(int bsem_id) {
  acquire(&bsem_lock);
  for (int i = 0; i < MAX_BSEM; i++) {
    if (binSemArray[i].id == bsem_id) { //found the semaphore
      while (binSemArray[i].is_locked==1) { //this semaphore is locked now
        sleep(&binSemArray[i],&bsem_lock); 
      }
      binSemArray[i].is_locked=1;
      release(&bsem_lock);
      return;
    }
  }
...
void bsem_up(int bsem_id){
  acquire(&bsem_lock);
  for (int i = 0; i < MAX_BSEM; i++) {
    if (binSemArray[i].id == bsem_id) { //found the semaphore
      if (binSemArray[i].is_locked==1) { //this semaphore is locked now
        binSemArray[i].is_locked=0;
        release(&bsem_lock);
        wakeup(&binSemArray[i]); 
        return;
      } 
    } 
  }
